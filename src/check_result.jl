...
# and gives a error message complaining about that
const LegacyZygoteCompTypes = Union{Tuple,NamedTuple}
check_equal(::C, expected::T; kwargs...) where {C<:Composite,T<:LegacyZygoteCompTypes} = @test C === T
check_equal(::T, expected::C; kwargs...) where {C<:Composite,T<:LegacyZygoteCompTypes} = @test T === C
